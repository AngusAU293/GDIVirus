...
namespace GDIVirus.Effects
{
    public class Slide
    {
        ...
        public Slide(int x, int y, int width, int height)
        {
            in_x = x;
            in_y = y;
            in_width = width;
        ...
        public void Start(int x_offset, int y_offset, int interval)
        {
            sliding = true;
            ...
            slideThread = new Thread(() =>
            {
                while (sliding)
                {
                    ...
                    BitBlt(hdc, in_x, in_y, in_width, in_height, hdc, in_x + x_offset, in_y + y_offset, TernaryRasterOperations.SRCCOPY);
                    DeleteDC(hdc);
